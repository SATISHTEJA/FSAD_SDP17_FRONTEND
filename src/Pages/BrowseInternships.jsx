...
const BrowseInternships = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://fsad-sdp17-backend-2.onrender.com/api/internships/all")
      .then((res) => res.json())
      .then(async (data) => {
        setInternships(data);
        const appliedList = [];
        for (let intern of data) {
          const res = await fetch(
            `https://fsad-sdp17-backend-2.onrender.com/api/applications/check?studentId=${userId}&internshipId=${intern.id}`
          );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        "https://fsad-sdp17-backend-2.onrender.com/api/applications/apply",
        {
          method: "POST",
          body: formData,
        }
